...
class Role extends Model
{
    ...
    public function scopeGetRoleAccess($query, $datas)
    {
        ...
        $query = DB::table("rbac_role_permission AS roleperm")
            ->select(DB::raw("CONCAT(perm.slug,' ',perm.access) as slugaccess"))
            ->join("rbac_permission AS perm", "perm.id", "=", "roleperm.permission_id")
    ...
    {
        $query = DB::table("rbac_access_name AS acname")
            ->select('name')
            ->orderBy('id')
            ->get();
    ...
    {
        $query = DB::table("rbac_permission")
            ->select('id AS permid')
            ->where('slug', $slug)
            ->where('access', $access)
            ->where('status', 1)
    ...
    {
        $query = DB::table("rbac_role_permission")->insert($data);
    ...
    {
        $query = DB::table("rbac_role_permission")
