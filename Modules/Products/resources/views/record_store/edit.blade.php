  ...
  <form action="{{ url()->current() }}" method="post" enctype="multipart/form-data">
    ...
    <div class="card-body row">
      <div class="col-sm-12">
        ...
        <div class="form-group">
          <label for="recordstore_name">Record Store Name</label>
          <input type="text" class="form-control" id="recordstore_name" name="recordstore_name" placeholder="Record Store Name" value="{{ $current->name }}" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <select class="select2bs4" multiple="multiple" data-placeholder="Pick genre" name="genre[]" style="width: 100%;">
            @foreach ($genre as $item)
                <option value="{{ $item->uuid }}" @selected(in_array($item->uuid, $rclbgenreSelected))>
                    {{ $item->name }}
                </option>
            @endForeach
          </select>
        ...
        <div class="form-group">
          ...
          <select class="select2bs4" multiple="multiple" data-placeholder="Pick music format" name="musicformat[]" style="width: 100%;">
            @foreach ($musicformat as $item)
                <option value="{{ $item->uuid }}" @selected(in_array($item->uuid, $rclbmusicformatSelected))>
                    {{ $item->name }}
                </option>
            @endForeach
          </select>
    ...
    <div class="card-footer">
      ...
      <a href="{{ url('products/recordstore') }}" onclick="return confirm('Anda yakin mau kembali?')" class="btn btn-success">Kembali</a>
    </div>
  </form>
</div>
